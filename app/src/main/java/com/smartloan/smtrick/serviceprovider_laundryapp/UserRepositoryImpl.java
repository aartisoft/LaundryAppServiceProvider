...
import static com.smartloan.smtrick.serviceprovider_laundryapp.Constant.EMAIL_POSTFIX;
...
public class UserRepositoryImpl extends FirebaseTemplateRepository implements UserRepository {
    ...
    @Override
    public void readServiceProviderByMobileNumber(final String number, final CallBack callback) {
        final Query query = Constant.SERVICE_PROVIDER_TABLE_REF.orderByChild("number").equalTo(number);
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot != null && dataSnapshot.getValue() != null && dataSnapshot.hasChildren()) {
